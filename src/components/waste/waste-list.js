        ...
        {wastes.map((waste) => (
          <Grid item xs={12} key={waste.id}>
            ...
            <Typography component="p">
              Owner - full name : {waste.owner}
            </Typography>
            <Typography component="p">
              Price : {waste.price.parseInt()}
            </Typography>
            <Typography component="p">City : {waste.city}</Typography>
            <Typography component="p">State : {waste.state}</Typography>
            <Typography component="p">
              Postal Code : {waste.postalCode.substring(0, 5)}
            </Typography>
            <Typography component="p">
              {' '}
              Date Accepted : {waste.dateAccepted.substring(0, 10)}
            </Typography>
            <Typography component="p">
              {' '}
              Date Returned - if applicable :{' '}
              {waste.dateReturned.substring(0, 10)}
            </Typography>
            <Button value={waste.id}>Return</Button>
